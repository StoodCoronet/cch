...
struct Config {
    happy: Option<HappyConfig>,
    profiles: Vec<Profile>,
}

pub fn load_happy_config() -> Option<HappyConfig> {
...
/// Write the `[happy]` section to profiles.toml. If a `[happy]` section already
/// exists, it is replaced in-place; otherwise it is appended at the top.
pub fn write_happy_config(server: &str, token: &str) -> Result<()> {
    let path = config_path();
    ...
    doc["happy"] = toml_edit::Item::Table({
        let mut t = toml_edit::Table::new();
        t["server"] = toml_edit::value(server);
        t["token"] = toml_edit::value(token);
        t
    });
...
/// Remove the `[happy]` section from profiles.toml. No-op if none exists.
pub fn remove_happy_config() -> Result<()> {
    let path = config_path();
    let content = fs::read_to_string(&path).with_context(|| format!("read config {path:?}"))?;
    let mut doc = content
        .parse::<toml_edit::DocumentMut>()
