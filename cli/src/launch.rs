...
/// into `(server_url, token)`. The server URL is scheme + host.
pub fn parse_connection_url(url: &str) -> Result<(String, String)> {
    let url = url.trim();
    // Find scheme separator
    let scheme_end = url.find("://").ok_or_else(|| anyhow::anyhow!("URL must have a scheme (https://...)"))?;
    let scheme = &url[..scheme_end];
    let after_scheme = &url[scheme_end + 3..];

    // Find the first '/' to get the host
    let path_start = after_scheme.find('/').unwrap_or(after_scheme.len());
    let host = &after_scheme[..path_start];

    if host.is_empty() {
        anyhow::bail!("URL must include a hostname");
    }

    let server = format!("{}://{}", scheme, host);

    // Extract token from query string
    let query_start = url.find("?token=").ok_or_else(|| {
        anyhow::anyhow!("URL must contain '?token=' query parameter")
    })?;
    let token = url[query_start + 7..].to_string();
    if token.is_empty() {
        anyhow::bail!("Token is empty in connection URL");
    }
